...
def proper_query(user_input, userid, chatbotid):
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            {"role": "system",
             "content": "You are a helpful assistant and you have two jobs. 1)Given a chat history and the latest user question which might reference context in the chat history, formulate a contextualized standalone prompt which can be understood without the chat history, for RAG purposes.(Remove referential ambiguities by adding the reference from history) If it is in other language, translate the contextual query in English. Do NOT answer the user prompt, just reformulate it if needed and otherwise return it as is.2)Run the store_lead_info function when you have any of the parameters and still perform job "},
